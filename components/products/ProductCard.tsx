...
import { Star, ShoppingCart, Scale, Truck, Check } from "lucide-react";
    ...
    >
      ...
      {!isOutOfStock && (
        <CardFooter className="flex gap-2 pt-3 border-none bg-transparent">
          <Button
            className="flex-1 gap-1.5 text-xs font-semibold text-white border-none transition-colors"
            style={{
              backgroundColor: added ? "var(--kapru-teal)" : "var(--kapru-coral)",
            }}
            onMouseEnter={(e) => {
              if (added) return;
